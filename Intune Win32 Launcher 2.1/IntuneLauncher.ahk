...
;@Ahk2Exe-SetDescription Intune Win32 Launcher 2.1
...
if ArgumentsNumber > 0
				...
				Loop
					...
					if (SectionType = "Notification" or SectionType = "Progress" or SectionType = "Message" or Section = "*" or Section = "?")
						...
						if SectionType = "Notification"
							...
							NotifyGui.SetFont("S12 bold", "Segoe UI")
		...
		if (SectionType = "Execution")
			...
			if WinLogonEnabled = true
				...
				if NotifyProgress
					...
					NotifyProgress := WinlogonGui.AddProgress("x" (A_ScreenWidth//4) . " y+20 w" (A_ScreenWidth//2) . " h20 -Smooth 0x8 c3399FF Background2C2C2C")
	...
	if FileExist(Inifile)
		...
		if SetupFilePath
			...
			EditGui := Gui("+DpiScale", ProductName " " ProductVersion " - Edit INI file")
			if AppsUseLightTheme = 0
				{
			...
			AuthorEdit := EditGui.Add("Edit", "x+5 w495 R1", A_UserName)
			...
			RegistrationEdit := EditGui.Add("Edit", "x+5 w495 R1", "HKEY_LOCAL_MACHINE\SOFTWARE\Intune Win32 Launcher\Packages")
			...
			LogFolderEdit := EditGui.Add("Edit", "x+5 w495 R1", "%ProgramData%\Microsoft\IntuneManagementExtension\Logs")
			...
			LogNameEdit := EditGui.Add("Edit", "x+5 w495 R1", LogFileName)
...
AddInstaller(*)
	...
	if DirExist(A_ScriptDir "\" WorkingDir)
		{
		SetupFilePath := FileSelect(3, A_ScriptDir "\" WorkingDir, "Select a Windows Installer or Package file (exe, msi, mst, appv, msix, appx)", "(*.exe; *.msi; *.mst; *.appv; *.msix; *.appx")
		if SetupFilePath
			{
			ReservedCharacters := "<>:;`"/\|?*,|"
			SplitPath SetupFilePath, &SetupFileName, &Path, &Extension, &SetupFileNameNoExt
			...
			if Extension = "msi"
				...
				LogFileName := StrReplace(LogFileName, ".", "_")
				if TransformFileName
					InstallCommand := "|" SetupFileName "|TRANSFORMS=`"" TransformFileName "`" TRANSFORMSSECURE=`"1`" REBOOT=`"ReallySuppress`"|%LogFolder%\" LogFileName "_MSI_Install.log"
				else
					InstallCommand := "|" SetupFileName "|REBOOT=`"ReallySuppress`"|%LogFolder%\" LogFileName "_MSI_Install.log"
			...
			if (Extension = "msix" OR Extension = "appx")
				...
				UnInstallCommand := ">Remove-AppPackage -AllUsers -package '" SetupFileNameNoExt "'"
				icon := 2
				col1 := "PS"
				}
			InstallLines++
...
CloseApp(*)
	...
	if EditList = true
		{
		Save := MsgBox("Would you like to save the INI file?", ProductName " " ProductVersion, "YesNo")
		if Save = "Yes"
			{
