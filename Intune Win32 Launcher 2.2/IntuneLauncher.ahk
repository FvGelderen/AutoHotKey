...
if ArgumentsNumber > 0
						...
						if (SectionType = "Notification" or SectionType = "Progress" or SectionType = "Message" or Section = "*" or Section = "?")
							...
							if SectionType = "Message"
								...
								if ShowWarning = 0
									...
									InformationText := IniRead(Inifile, Section, "InformationText", "")
									InformationText := TransForm(InformationText)
									if (Status = "Success" OR Status = "Warning" OR Status = "Error" OR Status = "Info")
										{
										if Status = "Success"
											{
										...
										if Status = "Info"
											{
											ShowIconNumber := "Icon5"
											BarColor := "Background3399FF"
											ProgressColor := "c3399FF"
											}
										}
									else
										{
										ShowIconNumber := "Icon1"
										BarColor := "Background646464"
										ProgressColor := "c646464"
										}
									}
								else
									{
			...
			if WinLogonEnabled = true
				...
				if NotifyProgress
					...
					if FileExist(TitleIcon)
						...
						ShowIcon := WinlogonGui.Add("Picture", "x" ((A_ScreenWidth - 64) // 2) . " y100 w64 h64 BackgroundTrans vIcon Icon1", A_ScriptFullPath)
					...
					NotifyPercentage := WinlogonGui.AddProgress("x" (A_ScreenWidth//4) . " y+10 w" (A_ScreenWidth//2) . " h20 -Smooth c646464 Background2C2C2C")
					NotifyProgress := WinlogonGui.AddProgress("x" (A_ScreenWidth//4) . " yp+20 w" (A_ScreenWidth//2) . " h20 -Smooth 0x8 c646464 Background000000")
	...
	if FileExist(Inifile)
		...
		{
		; --------------------------------------------------------------- Create INI File ---------------------------------------------------------------
		SetupFilePath := FileSelect(3, A_ScriptDir, "Please select the main Windows Installer or Package file (exe, msi, appv, msix, appx) that is located in a subfolder.", "(*.exe; *.msi; *.appv; *.msix; *.appx")
		if SetupFilePath
			{
			ReservedCharacters := "<>:;`"/\|?*,|"
			...
			if WorkingDir
				...
				if FoundPos := InStr(WorkingDir, "\")
					{
					SubFolder := SubStr(WorkingDir, FoundPos)
					SubFolder := LTrim(SubFolder, "\")
					WorkingDir := SubStr(WorkingDir, 1, FoundPos)
				...
				{
				MsgBox "Selected Windows Installer or Package file can not be used. Please select a Windows Installer or Package file from a subfolder relative to this program.", ProductName " " ProductVersion, "48 T30"
				ExitApp 1
				}
			if Extension = "msi"
				{
				...
				if (FileExist(Path . "\*.msp") OR FileExist(Path . "\*.mst"))
					{
					ApplyMsiFiles := FileSelect("M3", Path, "Select a patch (*.msp) and/or transform (*.mst) file(s) for " SetupFileName ".", "Windows Installer (*.mst;*.msp)")
					if ApplyMsiFiles
						{
			...
			if Extension = "exe"
				...
				if GetProductName = "Microsoft Office"
					{
					XmlFile := FileSelect(3, Path, "Select the Microsoft Office Installation Configuration file for " SetupFileName ".", "Office Configuration (*.xml)")
					if XmlFile
						{
						SplitPath(XmlFile, &XmlFileName,,)
						if SubFolder
							ExeInstallSwitch := "/configure `".\" SubFolder "\" XmlFileName "`""
						else
							ExeInstallSwitch := "/configure `"" XmlFileName "`""
						}
					XmlFile := FileSelect(3, Path, "Select the Microsoft Office Uninstallation Configuration file for " SetupFileName ".", "Office Configuration (*.xml)")
					if XmlFile
						{
						SplitPath(XmlFile, &XmlFileName,,)
						if SubFolder
							ExeUnInstallSwitch := "/configure `".\" SubFolder "\" XmlFileName "`""
						else
							ExeUnInstallSwitch := "/configure `"" XmlFileName "`""
						}
					}
				if GetProductName = "Microsoft Edge Update"
					{
					ExeInstallSwitch := "/silent /install"
					ExeUnInstallSwitch := ""
					}
				if InStr(GetProductName, "Microsoft .NET Framework")
					{
					ExeInstallSwitch := "/q /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /q /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "Microsoft Windows Desktop Runtime")
					{
					ExeInstallSwitch := "/install /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "Microsoft ASP.NET Core")
					{
					ExeInstallSwitch := "/install /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "Microsoft .NET Core Runtime")
					{
					ExeInstallSwitch := "/install /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "Microsoft .NET Core SDK")
					{
					ExeInstallSwitch := "/install /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "Microsoft Visual C++")
					{
					ExeInstallSwitch := "/install /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_Install.log`""
					ExeUnInstallSwitch := "/uninstall /quiet /norestart /log `"%LogFolder%\%LogFileName%_EXE_UnInstall.log`""
					}
				if InStr(GetProductName, "FileZilla")
					{
					ExeInstallSwitch := "/S /user=all /D=%ProgramFiles%\FileZilla FTP Client"
					ExeUnInstallSwitch := "%ProgramFiles%\FileZilla FTP Client\uninstall.exe /S"
					}
				MsgBox "Executable setup used. Please make sure the unattended switches that are used for installing and removing the selected package file are correct. You can edit the command lines for installing and removing the package file.", ProductName " " ProductVersion, "64 T30"
				}
			if AppsUseLightTheme = 0
				{
				BackColor := "2E2E2E"
			...
			if Extension = "exe"
				...
				if SubFolder
					IniWrite ".\" SubFolder "\" SetupFileName " " ExeInstallSwitch, Inifile, "Install", "Command1"
				else
					IniWrite SetupFileName " " ExeInstallSwitch, Inifile, "Install", "Command1"
				...
				if SubFolder
					IniWrite ".\" SubFolder "\" SetupFileName " " ExeUnInstallSwitch, Inifile, "UnInstall", "Command1"
				else
					IniWrite SetupFileName " " ExeUnInstallSwitch, Inifile, "UnInstall", "Command1"
			...
			FileAppend "`r`n", Inifile
			if !FileExist(A_ScriptDir . "\psScript.dll")
				FileInstall "psScript.dll", A_ScriptDir . "\psScript.dll", 1
	...
	if DirExist(A_ScriptDir "\" WorkingDir)
		...
		if SetupFilePath
			...
			if Extension = "exe"
				...
				if SubFolder
					{
					InstallCommand := ".\" SubFolder "\" SetupFileName " " ExeInstallSwitch
					UnInstallCommand := ".\" SubFolder "\" SetupFileName " " ExeUnInstallSwitch
					}
				else
					{
					InstallCommand := SetupFileName " " ExeInstallSwitch
					UnInstallCommand := SetupFileName " " ExeUnInstallSwitch
					}
				icon := 3
				col1 := "EXE"
				}
...
; --------------------------------------------------------------- Function for creating a per-session single-instance mutex. ---------------------------------------------------------------
CreateSingleInstanceMutex()
	{
	MutexName := "Global\IntuneLauncher.Installation"
...
class PsScriptManager
	...
	static Register(SetInstallDir := A_ScriptDir)
		{
	...
	static Unregister(SetInstallDir := A_ScriptDir)
		{
